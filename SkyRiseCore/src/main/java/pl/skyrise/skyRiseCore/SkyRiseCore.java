...
public class SkyRiseCore extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (CitizensHook.isEnabled()) {
            getLogger().info("Citizens połączony — NPC modułów dostępne.");
        } else {
            getLogger().info("Citizens nie znaleziony — używane będą natywne NPC/fallbacki.");
        }

        getServer().getPluginManager().registerEvents(messageCache, this);
        getServer().getPluginManager().registerEvents(npcRegistry, this);
        getServer().getPluginManager().registerEvents(new pl.skyrise.skyRiseCore.gui.GuiListener(), this);
