...
public class ChatModule implements Module {
    ...
    public void onEnable() {
        ...
        plugin.getCommand("globalchat").setExecutor(command);
    ...
    public void onDisable() {
        tabRegistry.unregister("globalchat");
        if (config != null) {
            config.save();
        }
    }
    ...
    private void cacheConfig() {
        ...
        this.globalFormat = config.getConfig().getString("global-format",
                "<white>[<aqua>G</aqua>]</white><reset> {prefix}<reset> <gray>{player}<reset> <dark_gray>»<reset> {group_color}{message}");

        this.cooldownMessage = config.getConfig().getString("cooldown-message",
                "<red>»</red> <red>Musisz odczekać jeszcze {time} sekund przed ponownym użyciem czatu globalnego.");
