    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (cmdName) {
            case "setfreeze" -> {
                ...
                if (!player.isOp()) {
                    player.sendMessage(ColorUtil.mini("<red>» Nie masz uprawnień! Tylko operatorzy (OP) mogą ustawiać punkt freeze."));
                    return true;
                }
            ...
            case "delfreeze" -> {
                if (!sender.isOp()) {
                    sender.sendMessage(ColorUtil.mini("<red>» Nie masz uprawnień! Tylko operatorzy (OP) mogą usuwać punkt freeze."));
                    return true;
                }
            ...
            case "freeze" -> {
                ...
                if (args.length == 0) {
                    sender.sendMessage(ColorUtil.mini("<#99CCFF>»</#99CCFF> <yellow>Użyj: /freeze <gracz>"));
                    return true;
                }
                ...
                if (module.isFrozen(uuid)) {
                    sender.sendMessage(ColorUtil.mini("<red>» Gracz <white>" + (target.getName() != null ? target.getName() : targetName) + "</white> jest już zamrożony."));
                    return true;
                }
                ...
                broadcastToStaff("<red>» Gracz <white>" + (target.getName() != null ? target.getName() : targetName) + "</white><red> został zamrożony przez <white>" + sender.getName() + "</white>.");
            }
            case "unfreeze" -> {
                if (!sender.hasPermission(module.getPermission())) {
                    sender.sendMessage(ColorUtil.mini("<red>» Nie masz uprawnień!"));
                ...
                if (args.length == 0) {
                    sender.sendMessage(ColorUtil.mini("<#99CCFF>»</#99CCFF> <yellow>Użyj: /unfreeze <gracz>"));
                    return true;
                }
                ...
                if (!module.isFrozen(uuid)) {
                    sender.sendMessage(ColorUtil.mini("<red>» Gracz <white>" + (target.getName() != null ? target.getName() : targetName) + "</white> nie jest zamrożony."));
                    return true;
                }

                module.unfreezePlayer(uuid);
                broadcastToStaff("<green>» Gracz <white>" + (target.getName() != null ? target.getName() : targetName) + "</white> został odmrożony przez <white>" + sender.getName() + "</white>.");
            }
        }
