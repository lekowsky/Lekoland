...
public class KnockoutListener implements Listener {
    ...
    public void onDamage(EntityDamageEvent event) {
        ...
        // 2. Jeśli gracz miałby zginąć od tych obrażeń — ratujemy go i wprowadzamy w stan powalenia
    ...
    public void onCommand(PlayerCommandPreprocessEvent event) {
        if (module.isKnocked(event.getPlayer().getUniqueId())) {
            event.setCancelled(true);
            event.getPlayer().sendMessage(ColorUtil.mini("<red>»</red> <red>Nie możesz używać komend podczas powalenia!"));
        }
    }
