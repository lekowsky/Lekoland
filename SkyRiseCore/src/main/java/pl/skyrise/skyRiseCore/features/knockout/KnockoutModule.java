...
public class KnockoutModule implements Module {
    ...
    public void onEnable() {
        ...
        plugin.getServer().getPluginManager().registerEvents(new KnockoutListener(this), plugin);
    ...
    private void cacheConfig() {
        ...
        this.msgKnockedSelf = config.getConfig().getString("messages.knocked-self", "<red>»</red> <red>Zostałeś/aś powalony/a! Masz {time}s na ratunek. Kliknij 3x Shift aby się wykrwawić.");
        this.msgKnockedBroadcast = config.getConfig().getString("messages.knocked-broadcast", "<red>»</red> <red>Gracz <white>{player}</white> został powalony i potrzebuje pomocy!");
        this.msgRevivingActionbar = config.getConfig().getString("messages.reviving-actionbar", "<gold>Podnoszenie: </gold>{bar}");
        this.msgRevivedSelf = config.getConfig().getString("messages.revived-self", "<green>»</green> <green>Zostałeś/aś podniesiony/a przez <white>{player}</white>!");
        this.msgRevivedOther = config.getConfig().getString("messages.revived-other", "<green>»</green> <green>Podniosłeś/aś gracza <white>{player}</white>!");
        this.msgBleedoutBroadcast = config.getConfig().getString("messages.bleedout-broadcast", "<red>»</red> <red>Gracz <white>{player}</white> wykrwawił się na śmierć.");
    }
    ...
    public void revivePlayer(Player reviver, Player target) {
        ...
        target.sendActionBar(ColorUtil.mini("<green>»</green> <green>Zostałeś/aś podniesiony/a!"));
        reviver.sendActionBar(ColorUtil.mini("<green>»</green> <green>Podniosłeś/aś gracza!"));
    ...
    public void handleKnockedSneak(Player player) {
        ...
        if (revivingTargets.containsValue(uuid)) {
            player.sendMessage(ColorUtil.mini("<red>»</red> <red>Nie możesz się wykrwawić, ponieważ ktoś Cię podnosi!"));
            return;
        }
        ...
        if (count >= 3) {
            ...
            player.sendMessage(ColorUtil.mini("<red>»</red> <red>Kliknij Shift jeszcze " + (3 - count) + "x aby się wykrwawić."));
        }
    }
