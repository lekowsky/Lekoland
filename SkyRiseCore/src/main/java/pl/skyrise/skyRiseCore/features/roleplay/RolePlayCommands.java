...
public class RolePlayCommands implements CommandExecutor {
    ...
    private void handleMe(Player player, String[] args) {
        if (args.length == 0) {
            player.sendMessage(ColorUtil.mini("<#99CCFF>»</#99CCFF> <yellow>Użycie: /me <akcja>"));
            return;
        }
    ...
    private void handleDo(Player player, String[] args) {
        if (args.length == 0) {
            player.sendMessage(ColorUtil.mini("<#99CCFF>»</#99CCFF> <yellow>Użycie: /do <opis otoczenia>"));
            return;
        }
    ...
    private void handleTry(Player player, String[] args) {
        if (args.length == 0) {
            player.sendMessage(ColorUtil.mini("<#99CCFF>»</#99CCFF> <yellow>Użycie: /try <próba akcji>"));
            return;
        }
    ...
    private void handleOpis(Player player, String[] args) {
        ...
        if (args.length == 0) {
            player.sendMessage(ColorUtil.mini("<#99CCFF>»</#99CCFF> <yellow>Użycie: /opis <treść opisu postaci>"));
            return;
        }
    ...
    private void handleOpis2(Player player, String[] args) {
        ...
        if (args.length >= 2 && args[0].equalsIgnoreCase("time")) {
            ...
            try {
                ...
                player.sendMessage(ColorUtil.mini("<green>» Opis miejsca został ustawiony na " + durationText + ".</green>"));
        ...
        if (args.length == 0) {
            player.sendMessage(ColorUtil.mini("<#99CCFF>»</#99CCFF> <yellow>Użycie: /opis2 <opis miejsca stojącego>"));
            return;
        }
